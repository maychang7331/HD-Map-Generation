...
def placeholder_inputs(batch_size, num_point, num_features):
    # placeholder 讓 computational graph 保留輸入欄位的節點，其允許實際的輸入值留到後來再指定,
    # 即讓模型透過 placeholder 輸入各種的資料
...
def get_model(point_cloud, is_training, bn_decay=None):
    ...
    input_image = tf.expand_dims(point_cloud, -1)   # 給point_cloud增加個維度 (16, 4096, 7, 1) 即input image 的channel數為1
    # CONV  # inputs, num_output_channels, kernel_size
...
if __name__ == "__main__":
    with tf.Graph().as_default():
        a = tf.placeholder(tf.float32, shape=(32,4096,7))
        ...
        with tf.Session() as sess:
            ...
            for i in range(100):
                print(i)
                sess.run(net, feed_dict={a:np.random.rand(32,4096,7)})
